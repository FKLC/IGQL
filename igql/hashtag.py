...
class Hashtag:
    def __init__(self, data, igql, fetch_data=False):
        ...
        self._recent_media_has_next_page = self.data['edge_hashtag_to_media'][
            'page_info']['has_next_page']
        self._recent_media_cursor = self.data['edge_hashtag_to_media']['page_info'][
            'end_cursor']
    ...
    def iterate_more_recent_media(self, reset=False, fetch_data=False):
        if reset:
            self._recent_media_has_next_page = self.data['edge_hashtag_to_media'][
                'page_info']['has_next_page']
            self._recent_media_cursor = self.data['edge_hashtag_to_media'][
